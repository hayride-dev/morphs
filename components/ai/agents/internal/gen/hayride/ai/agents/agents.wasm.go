...
//go:wasmexport hayride:ai/agents@0.0.61#[method]agent.invoke
//export hayride:ai/agents@0.0.61#[method]agent.invoke
func wasmexport_AgentInvoke(self0 uint32, input0 uint32, input1 *types.MessageContent, input2 uint32) (result *cm.Result[cm.List[Message], cm.List[Message], Error]) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	input := lift_Message((uint32)(input0), (*types.MessageContent)(input1), (uint32)(input2))
	result_ := Exports.Agent.Invoke(self, input)
	result = &result_
	return
}

//go:wasmexport hayride:ai/agents@0.0.61#[method]agent.invoke-stream
//export hayride:ai/agents@0.0.61#[method]agent.invoke-stream
func wasmexport_AgentInvokeStream(self0 uint32, message0 uint32, message1 *types.MessageContent, message2 uint32, writer0 uint32) (result *cm.Result[Error, struct{}, Error]) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
