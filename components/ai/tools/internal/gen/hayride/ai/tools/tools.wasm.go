// Code generated by wit-bindgen-go. DO NOT EDIT.

package tools

import (
	"go.bytecodealliance.org/cm"
)

// This file contains wasmimport and wasmexport declarations for "hayride:ai@0.0.60".

//go:wasmimport [export]hayride:ai/tools@0.0.60 [resource-new]error
//go:noescape
func wasmimport_ErrorResourceNew(rep0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:ai/tools@0.0.60 [resource-rep]error
//go:noescape
func wasmimport_ErrorResourceRep(self0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:ai/tools@0.0.60 [resource-drop]error
//go:noescape
func wasmimport_ErrorResourceDrop(self0 uint32)

//go:wasmexport hayride:ai/tools@0.0.60#[dtor]error
//export hayride:ai/tools@0.0.60#[dtor]error
func wasmexport_ErrorDestructor(self0 uint32) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	Exports.Error.Destructor(self)
	return
}

//go:wasmexport hayride:ai/tools@0.0.60#[method]error.code
//export hayride:ai/tools@0.0.60#[method]error.code
func wasmexport_ErrorCode(self0 uint32) (result0 uint32) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	result := Exports.Error.Code(self)
	result0 = (uint32)(result)
	return
}

//go:wasmexport hayride:ai/tools@0.0.60#[method]error.data
//export hayride:ai/tools@0.0.60#[method]error.data
func wasmexport_ErrorData(self0 uint32) (result *string) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	result_ := Exports.Error.Data(self)
	result = &result_
	return
}

//go:wasmimport [export]hayride:ai/tools@0.0.60 [resource-new]tools
//go:noescape
func wasmimport_ToolsResourceNew(rep0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:ai/tools@0.0.60 [resource-rep]tools
//go:noescape
func wasmimport_ToolsResourceRep(self0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:ai/tools@0.0.60 [resource-drop]tools
//go:noescape
func wasmimport_ToolsResourceDrop(self0 uint32)

//go:wasmexport hayride:ai/tools@0.0.60#[dtor]tools
//export hayride:ai/tools@0.0.60#[dtor]tools
func wasmexport_ToolsDestructor(self0 uint32) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	Exports.Tools.Destructor(self)
	return
}

//go:wasmexport hayride:ai/tools@0.0.60#[constructor]tools
//export hayride:ai/tools@0.0.60#[constructor]tools
func wasmexport_Constructor() (result0 uint32) {
	result := Exports.Tools.Constructor()
	result0 = cm.Reinterpret[uint32](result)
	return
}

//go:wasmexport hayride:ai/tools@0.0.60#[method]tools.call
//export hayride:ai/tools@0.0.60#[method]tools.call
func wasmexport_ToolsCall(self0 uint32, input0 *uint8, input1 uint32, input2 *uint8, input3 uint32, input4 *uint8, input5 uint32, input6 *[2]string, input7 uint32) (result *cm.Result[ToolOutputShape, ToolOutput, ErrorCode]) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	input := lift_ToolInput((*uint8)(input0), (uint32)(input1), (*uint8)(input2), (uint32)(input3), (*uint8)(input4), (uint32)(input5), (*[2]string)(input6), (uint32)(input7))
	result_ := Exports.Tools.Call(self, input)
	result = &result_
	return
}

//go:wasmexport hayride:ai/tools@0.0.60#[method]tools.capabilities
//export hayride:ai/tools@0.0.60#[method]tools.capabilities
func wasmexport_ToolsCapabilities(self0 uint32) (result *cm.Result[cm.List[ToolSchema], cm.List[ToolSchema], Error]) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	result_ := Exports.Tools.Capabilities(self)
	result = &result_
	return
}
